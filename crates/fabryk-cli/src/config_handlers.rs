...
pub fn handle_config_command(config_path: Option<&str>, action: ConfigAction) -> Result<()> {
    match action {
        ...
        ConfigAction::Get { key } => {
            cmd_config_get_or_dump::<FabrykConfig>(config_path, key.as_deref())
        }
        ...
        ConfigAction::Export { docker_env, file } => {
            let config = FabrykConfig::load(config_path)?;
            cmd_config_export(&config, docker_env, file.as_deref())
        }
    }
}
...
/// Get a configuration value by dotted key, or dump the full config.
///
/// When `key` is `None`, prints the full configuration as TOML.
/// When `key` is `Some`, prints the specific value at that dotted path.
pub fn cmd_config_get_or_dump<C: ConfigManager>(
    config_path: Option<&str>,
    key: Option<&str>,
) -> Result<()> {
    match key {
        Some(k) => cmd_config_get::<C>(config_path, k),
        None => {
...
///
/// When `file` is `Some`, writes to that file with a header comment.
/// When `file` is `None`, prints to stdout.
///
/// The `docker_env` flag formats output as `--env KEY=VALUE` (for Docker CLI)
/// when writing to stdout. When writing to a file, the format is always
/// `KEY=VALUE` (standard `.env` format).
pub fn cmd_config_export<C: ConfigManager>(
    config: &C,
    docker_env: bool,
    file: Option<&str>,
) -> Result<()> {
    ...
    if let Some(path) = file {
        ...
        writeln!(
            f,
            "# Regenerate with: {} config export --docker-env --file {path}",
            C::project_name()
        )
        .map_err(|e| Error::config(e.to_string()))?;
        writeln!(f).map_err(|e| Error::config(e.to_string()))?;

        for (key, value) in &vars {
            writeln!(f, "{key}={value}").map_err(|e| Error::config(e.to_string()))?;
        }

        println!("Wrote {} variables to {path}", vars.len());
    } else {
        for (key, value) in &vars {
            if docker_env {
...
mod tests {
    ...
    fn test_cmd_config_export_env_vars() {
        let config = FabrykConfig::default();
        let result = cmd_config_export(&config, false, None);
        assert!(result.is_ok());
    }
    ...
    fn test_cmd_config_export_docker_env() {
        let config = FabrykConfig::default();
        let result = cmd_config_export(&config, true, None);
        assert!(result.is_ok());
    }

    #[test]
    fn test_cmd_config_export_to_file() {
        let dir = tempfile::TempDir::new().unwrap();
        let env_path = dir.path().join(".env");
        let config = FabrykConfig::default();

        let result = cmd_config_export(&config, false, Some(env_path.to_str().unwrap()));
        assert!(result.is_ok());
        assert!(env_path.exists());

        let content = std::fs::read_to_string(&env_path).unwrap();
        assert!(content.contains("Generated by fabryk"));
        assert!(content.contains("FABRYK_PROJECT_NAME=fabryk"));
    }

    #[test]
    fn test_cmd_config_get_or_dump_full() {
        let dir = tempfile::TempDir::new().unwrap();
        let path = dir.path().join("config.toml");
        let config = FabrykConfig::default();
        std::fs::write(&path, config.to_toml_string().unwrap()).unwrap();

        let result = cmd_config_get_or_dump::<FabrykConfig>(Some(path.to_str().unwrap()), None);
        assert!(result.is_ok());
    }

    #[test]
    fn test_cmd_config_get_or_dump_with_key() {
        let dir = tempfile::TempDir::new().unwrap();
        let path = dir.path().join("config.toml");
        let config = FabrykConfig::default();
        std::fs::write(&path, config.to_toml_string().unwrap()).unwrap();

        let result = cmd_config_get_or_dump::<FabrykConfig>(
            Some(path.to_str().unwrap()),
            Some("project_name"),
        );
        assert!(result.is_ok());
